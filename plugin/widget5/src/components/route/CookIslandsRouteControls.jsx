...
// Draw/undo/clear/run controls for a Cook Islands route forecast. Trimmed
// from widget1's RouteForecastControls.jsx: no GPX/GeoJSON file import --
// this app only draws a route by clicking the map.
function CookIslandsRouteControls({
  routePoints,
  routePickMode,
  setRoutePickMode,
  routeSpeedKt,
  ...
  // designator) -- the same convention cookIslandsRouteForecastService.js's
  // parseAsUtcWallClock relies on. These *Local values are that same
  // canonical format, used only for the clamp comparisons below; what the
  // picker actually *displays* is a separate zone-converted value further
  // down, so switching the timezone toggle never changes what gets sent to
  // the backend.
  ...
  // wall-clock strings for whichever zone is currently selected, and
  // convert the user's edit straight back to UTC on change.
  ...
  // forecast window was known, and not every browser enforces datetime-
  // local's min/max the same way.
  useEffect(() => {
  ...
  // the user always knows what clicking the map will do next.
  const workflowHint = routePickMode
    ...
    : pointCount === 0
      ? 'Click "Draw" to start plotting a route.'
      : pointCount === 1
    ...
    <div className="map-display-option">
      <div className="map-display-option__segmented" role="group" aria-label="Route drawing actions">
        ...
        >
          ...
          {routePickMode ? 'Click map…' : 'Draw route'}
        </button>
      </div>
      ...
      <div className="map-display-option__hint">{workflowHint}</div>

      {routeForecastError && (
        <div className="map-display-option__hint" style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-start' }}>
          <span style={{ color: '#f87171' }}>{routeForecastError}</span>
          {retryable && (
            <button type="button" className="map-display-option__btn" onClick={onRunRouteForecast}>
...
export default CookIslandsRouteControls;
